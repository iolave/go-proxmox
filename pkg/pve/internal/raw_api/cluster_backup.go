...
func ClusterBackupGetJobs(c *pve.Client) (res []ClusterBackupGetJobResponse, err error) {
...
func ClusterBackupDeleteJob(c *pve.Client, id string) (err error) {
	req := struct {
...
//
//	Check: ["perm","/",["Sys.Audit"]]
func ClusterBackupGetJob(c *pve.Client, id string) (res ClusterBackupGetJobResponse, err error) {
	req := struct {
		ID string `in:"path=id;nonzero"`
	}{
		ID: id,
...
//	Check: ["perm","/",["Sys.Modify"]]
func ClusterBackupPostJob(c *pve.Client, req ClusterBackupPostJobRequest) (err error) {
	err = c.Do(pve.Request{
		Path:    "/api2/json/cluster/backup",
...
//	The 'tmpdir', 'dumpdir' and 'script' parameters are additionally restricted to the 'root@pam' user.
//	Check: ["perm","/",["Sys.Modify"]]
func ClusterBackupUpdateJob(c *pve.Client, req ClusterBackupUpdateJobRequest) (err error) {
	err = c.Do(pve.Request{
		Path:    "/api2/json/cluster/backup/{id}",
...
//
//	Check: ["perm","/",["Sys.Audit"]]
func ClusterBackupGetJobIncludedVolumes(c *pve.Client, id string) (res struct {
	Children []struct {
		VMID     int    `json:"id"`
