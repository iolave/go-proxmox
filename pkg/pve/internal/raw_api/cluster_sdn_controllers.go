...
func ClusterSDNControllersList(c *pve.Client, pending, running int, t string) (res []struct {
...
func ClusterSDNControllersNewController(c *pve.Client, req ClusterSDNControllersNewControllerRequest) (err error) {
	err = c.Do(pve.Request{
		Path:    "/api2/json/cluster/sdn/controllers",
...
func ClusterSDNControllersGetController(c *pve.Client, controller string, pending, running int) (res struct {
	Controller string  `json:"controller"`
	Type       string  `json:"type"`
	Pending    *int    `json:"pending"`
	State      *string `json:"state"`
...
//
//	Check: ["perm","/sdn/controllers/{controller}",["SDN.Allocate"]]
func ClusterSDNControllersUpdateController(c *pve.Client, req ClusterSDNControllersUpdateControllerRequest) (err error) {
	err = c.Do(pve.Request{
		Path:    "/api2/json/cluster/sdn/controllers/{controller}",
...
//
//	Check: ["perm","/sdn/controllers",["SDN.Allocate"]]
func ClusterSDNControllersDeleteController(c *pve.Client, controller, lockToken string) (err error) {
	req := struct {
		Controller string `in:"path=controller;nonzero"`
		LockToken  string `in:"query=lock-token;omitempty"`
	}{
